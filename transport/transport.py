...
def get_init(
    ...
    **kwargs: dict,
    ...
    str_init = (
        offset.join(
            [f"(capacity v{i} c{random.randint(1, max_capacity)})" for i in range(1, 1 + vehicles)]
        )
        + offset
    )
    ...
    str_init += (
        offset.join([f"(capacity-predecessor c{i} c{i + 1})" for i in range(max_capacity)])
        + offset
    )
    ...
    str_init += (
        offset.join([f"(at p{i} l{origins[i - 1]})" for i in range(1, 1 + packages)]) + offset
    )

    # Vehicles are in random locations
    str_init += (
        offset.join([f"(at v{i} l{random.randint(1, locations)})" for i in range(1, 1 + vehicles)])
        + offset
    )
