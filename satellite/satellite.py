...
def get_init(
    ...
    **kwargs: dict,
    ...
    str_init = offset + offset.join(
        [f"(pointing sat{s} dir{random.randint(1,directions)})" for s in range(1, 1 + satellites)]
    )
    ...
    str_init += offset + offset.join(
        [f"(calibration_target ins{i} dir{random.randint(1,directions)})" for i in range(1, 1 + instruments)]
    )
    ...
    onboard_instruments.extend(
        [f"(on_board ins{i} sat{random.randint(1, satellites)})" for i in range(1 + satellites, 1 + instruments)]
    )
