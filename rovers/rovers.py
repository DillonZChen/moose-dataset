...
def get_objects(rovers: int, waypoints: int, cameras: int, objectives: int, **kwargs: dict) -> str:
    offset = "\n    "
    ...
    # -- waypoints
    str_objects += (
        offset + " ".join([get_waypoint(i) for i in range(1, 1 + waypoints)]) + " - waypoint"
    )
    ...
    # -- objectives
    str_objects += (
        offset + " ".join([get_objective(i) for i in range(1, 1 + objectives)]) + " - objective"
    )
...
def get_init(
    ...
    str_init += offset + offset.join(
        [f"(at {r} {get_waypoint(random.randint(1,waypoints))})" for r in all_rovers]
    )

    # Rover equipment: 50% chance to be in a rover; all of them supported by at least one rover
    random.shuffle(all_rovers)
    str_init += offset + offset.join(
        [f"(equipped_for_soil_analysis {r})" for r in all_rovers[0 : random.randint(1, rovers)]]
    )
    random.shuffle(all_rovers)
    str_init += offset + offset.join(
        [f"(equipped_for_rock_analysis {r})" for r in all_rovers[0 : random.randint(1, rovers)]]
    )
    random.shuffle(all_rovers)
    ...
    str_init += offset + offset.join(
        [f"(store_of {get_store(r)} {get_rover(r)})" for r in range(1, 1 + rovers)]
    )

    # 50% chance of rock and/or soil samples in each waypoint
    rock_samples = [
        get_waypoint(w) for w in range(1, 1 + waypoints) if random.choice([True, False])
    ]
    soil_samples = [
        get_waypoint(w) for w in range(1, 1 + waypoints) if random.choice([True, False])
    ]
    ...
    str_init += offset + offset.join(
        [f"(visible {get_waypoint(a)} {get_waypoint(b)})" for a, b in visible_graph]
    )
    ...
    for o in range(1, 1 + objectives):
        ...
        str_init += offset + offset.join(
            [f"(visible_from {get_objective(o)} {w})" for w in all_waypoints[0:num_w]]
        )
...
def get_goal(
    ...
    **kwargs: dict,
    ...
    all_objectives_modes = [
        (get_objective(o), m) for o in range(1, 1 + objectives) for m in get_modes()
    ]
